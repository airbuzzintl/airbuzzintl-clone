  ...
  return (
    <>
      <BackgroundImage>
        ...
        >
          {breadcrumbs("Home / Domestic Services")}
        </div>
        <div style={{ marginLeft: "2rem", paddingTop: "9rem" }}>
          <TextContainer
            value={"DOMESTIC DELIGHTS"}
            fontSize={"8vw"}
            fontWeight={"bold"}
            color={"white"}
            textShadow={
        ...
        <Grid container style={{ marginTop: "1rem" }}>
          ...
          >
            <LeftContainer>
              <LeftWrapper>
                <Marquee>
                  <MarqueeGroup hover={hover}>
                    {response?.service?.map((e, i) => (
                      <ServiceCard e={e} i={i} setMouseHover={setHover} />
                    ))}
                  </MarqueeGroup>
          ...
          >
            <LeftContainer>
              <RightWrapper>
                <Marquee>
                  <MarqueeGroup1 hover={hover1}>
                    {response?.service?.map((e, i) => (
                      <ServiceCard e={e} i={i} setMouseHover={setHover1} />
                    ))}
                  </MarqueeGroup1>
                  <MarqueeGroup1 aria-hidden="true" hover={hover1}>
                    {response?.service?.map((e, i) => (
                      <ServiceCard e={e} i={i} setMouseHover={setHover1} />
                    ))}
                  </MarqueeGroup1>
          ...
          >
            ...
            <div style={{ padding: "5%" }}>
              <TextContainer
                value={
                  select === null
                    ? response?.domestic_service_title
              ...
              {select === null ? (
                <>
                  <TextContainer
                    value={response?.domestic_service_Para_1}
                  ...
                  <TextContainer
                    value={response?.domestic_service_Para_2}
                    fontSize={fontSizes.subcontent}
                    color={skyExColors.textGrey}
                    textAlign={"justify"}
                    gutterBottom
                  ...
                  <div style={{ paddingTop: "1rem" }}>
                    {response?.service[select]?.path === "#estimation" ? (
                      ...
                      <FormButton
                        ...
                        fontWeight={"500"}
                        clickFunction={() => {
                          router.push(response?.service[select]?.path);
                        }}
                      />
                    )}
                  </div>
...
const Service = styled.div`
  ...
  justify-subtitle: space-between;
  cursor: pointer;
  margin: 0.3rem;
  :hover {
    opacity: 0.8;
