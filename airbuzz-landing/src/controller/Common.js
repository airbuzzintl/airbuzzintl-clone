...
import { fontSizes, skyExColors } from "./constant";
import axios from "axios";
import { downloadUrl } from "../utils/api";

export const mobileNumberFormat = /^\+?[1-9][0-9]{9}$/;
...
export const handleChange = (name, value, fn) => {
  fn((prev) => {
    return {
      ...prev,
      [name]: value,
...
export const handleDownload = async (file, name) => {
  try {
    const response = await axios.get(`${downloadUrl}${file}`);
    let headers = response.headers;
    let extension = headers.get("Content-type").split("/")[2];
...
export const breadcrumbs = (address, color) => {
  ...
  return (
    <Stack style={{ display: "flex", flexDirection: "row" }}>
      ...
      <TextContainer
        fontSize={fontSizes.careerCardContentDescription.sm}
        color={color ? color : skyExColors.secondary}
        value={`${data[1]}`}
      />
    </Stack>
  );
